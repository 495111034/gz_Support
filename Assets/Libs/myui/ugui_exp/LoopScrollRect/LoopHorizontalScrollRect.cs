...
namespace UnityEngine.UI
{
    ...
    public class LoopHorizontalScrollRect : LoopScrollRect
    {
        ...
        protected override bool UpdateItems(Bounds viewBounds, Bounds contentBounds)
        {
            ...
            else if (viewBounds.max.x < contentBounds.max.x - _threshold)
            {
                float size = DeleteItemAtEnd(), totalSize = size;
                while (size > 0 && viewBounds.max.x < contentBounds.max.x - _threshold - totalSize)
                {
            ...
            else if (viewBounds.min.x > contentBounds.min.x + _threshold)
            {
                float size = DeleteItemAtStart(), totalSize = size;
                while (size > 0 && viewBounds.min.x > contentBounds.min.x + _threshold + totalSize)
                {
