...
namespace SG
{
    ...
    class Pool
    {
        ...
        public Pool(string poolName, GameObject poolObjectPrefab, GameObject rootPoolObj, int initialCount, PoolInflationType type)
        {
            ...
            po.__id = go.GetInstanceID().ToString();
            AddObjectToPool(po);
        ...
        private void AddObjectToPool(PoolObject po)
        {
            ...
            po.isPooled = true;
            po.__id = po.gameObject.GetInstanceID().ToString();
            //add to a root obj
            po.gameObject.transform.SetParent(rootObj.transform, false);

//#if UNITY_EDITOR
           // Log.LogInfo($"增加 {po.name} 到缓存 :{rootObj.name},增加后的缓存数量：{availableObjStack.Count}");
//#endif
        }

        private PoolObject GetObjectFromPool()
        {
            var po = availableObjStack.Pop();
//#if UNITY_EDITOR
            //Log.LogInfo($"从缓存  {rootObj.name} 获取一个实例,最新缓存数量：{availableObjStack.Count}");
//#endif
            return po;
        }
