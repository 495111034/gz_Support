...
using InputType = UnityEngine.UI.InputField.InputType;
using CharacterValidation = UnityEngine.UI.InputField.CharacterValidation;
using LineType = UnityEngine.UI.InputField.LineType;
using OnValidateInput = UnityEngine.UI.InputField.OnValidateInput;


namespace UnityEngine.UI
{
    [AddComponentMenu("UI/My Input Field", 31)]
    public class MyInputField : Selectable,
    ...
    private IEnumerator OverlayHtmlCoroutine()
    {
        ...
        public InputField.EndEditEvent _onEndEdit { get; set; } = new InputField.EndEditEvent();

        void onEndEdit()
        {
            if (Application.isPlaying)
            {
                _onEndEdit?.Invoke(this.text);
                SendMessageUpwards("__OnInputEditEnd", this);
            }
        }
        ...
        protected MyText m_TextComponent;
        ...
        [SerializeField]
        public bool SendClickMessage = false;
        ...
        public bool shouldHideMobileInput
        {
            set
            {
                MySetPropertyUtility.SetStruct(ref m_HideMobileInput, value);
            }
            get
            {
        ...
        public float caretBlinkRate
        {
            ...
            set
            {
                if (MySetPropertyUtility.SetStruct(ref m_CaretBlinkRate, value))
                {
                    if (m_AllowInput)
                        SetCaretActive();
                }
        ...
        public MyText textComponent { get { return m_TextComponent; } set { MySetPropertyUtility.SetClass(ref m_TextComponent, value); } }

        public Graphic placeholder { get { return m_Placeholder; } set { MySetPropertyUtility.SetClass(ref m_Placeholder, value); } }

        public Color selectionColor { get { return m_SelectionColor; } set { MySetPropertyUtility.SetColor(ref m_SelectionColor, value); } }

        public int characterLimit { get { return m_CharacterLimit; } set { MySetPropertyUtility.SetStruct(ref m_CharacterLimit, value); } }

        // Content Type related

        public ContentType contentType { get { return m_ContentType; } set { if (MySetPropertyUtility.SetStruct(ref m_ContentType, value)) EnforceContentType(); } }

        public LineType lineType { get { return m_LineType; } set { if (MySetPropertyUtility.SetStruct(ref m_LineType, value)) SetToCustomIfContentTypeIsNot(ContentType.Standard, ContentType.Autocorrected); } }

        public InputType inputType { get { return m_InputType; } set { if (MySetPropertyUtility.SetStruct(ref m_InputType, value)) SetToCustom(); } }

        public TouchScreenKeyboardType keyboardType { get { return m_KeyboardType; } set { if (MySetPropertyUtility.SetStruct(ref m_KeyboardType, value)) SetToCustom(); } }

        public CharacterValidation characterValidation { get { return m_CharacterValidation; } set { if (MySetPropertyUtility.SetStruct(ref m_CharacterValidation, value)) SetToCustom(); } }

        // Derived property
        public bool multiLine { get { return m_LineType == LineType.MultiLineNewline || lineType == LineType.MultiLineSubmit; } }
        // Not shown in Inspector.
        public char asteriskChar { get { return m_AsteriskChar; } set { MySetPropertyUtility.SetStruct(ref m_AsteriskChar, value); } }
        public bool wasCanceled { get { return m_WasCanceled; } }

        private bool limitCharacterByByte = false;

        public void SetLimitCharacterByByte(bool isOn)
        {
            limitCharacterByByte = isOn;
        }
...
#else
            MyTask.Run(MoveNextIEnumerator(shift));
#endif
        }


#if UNITY_ASYNC
        async void MoveNextAsync(bool shift)
        {
            await MyTask.WaitForMilliseconds((int)(100));
...
#else
        IEnumerator MoveNextIEnumerator(bool shift)
        {
            yield return null;
            int position = text.Length;
        ...
        public void SetValue(string _str,bool isSendEvent = true)
        {
            if (isSendEvent)
                SendOnValueChangedAndUpdateLabel();
            else
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (m_ShouldActivateNextUpdate)
            {
                if (!isFocused)
                {
                    if (!ActivateInputFieldInternal()) return;
            ...
            if (m_Text != val)
            {
                ...
                for (int i = 0; i < val.Length; ++i)
                {
                    ...
                    if (m_OnValidateInput != null)
                        c = m_OnValidateInput(m_Text, m_Text.Length, c);
                ...
                if (limitCharacterByByte == true && CalcByteCareless(text) > characterLimit)
                {
                    m_Text = m_Text.Substring(0, SubMaxLength(text, characterLimit));
                }
        ...
        public static int SubMaxLength(string text, int maxByte)
        {
            ...
            {
                if (text[i] > 127 || (text[i] >= 65 && text[i] <= 90))
                {
                    byteNum += 2;
                }
                else
        ...
        /// <summary>
        /// 粗糙地计算字符串的字节宽度
        /// 所有非ASCII编码和大写字母都两个字节
        /// </summary>
        public static int CalcByteCareless(string text)
        {
            int byteNum = 0;
            for (int i = 0; i < text.Length; i++)
            {
        ...
        {
            onEndEdit();
//            Log.LogInfo($"endEdit-1,m_Text:{m_Text}, text:{text},value:{Value},minValue:{MinValue},maxValue:{MaxValue}");
            //对数字输入框进行检查
...
//            Log.LogInfo($"endEdit-2,m_Text:{m_Text}, text:{text},value:{Value},minValue:{MinValue},maxValue:{MaxValue}");
        }
        ...
        protected virtual void Append(char input)
        {
            ...
            if (m_OnValidateInput != null)
                input = m_OnValidateInput(text, caretPositionInternal, input);
            else if (characterValidation != CharacterValidation.None)
        ...
        private void GenerateCursor(VertexHelper vbo, Vector2 roundingOffset)
        {
            ...
            float width = 1f;
            float height = m_LineType == LineType.SingleLine? m_TextComponent.rectTransform.rect.height : m_TextComponent.fontSize;
        ...
        private bool ActivateInputFieldInternal()
        {
        ...
        /// 是否开启虚拟键盘弹出时上移界面
        /// </summary>
        public void CreateViewAdaptKeyBoard(RectTransform move_rect, float _up_offset = 0)
        {
            ChatViewAdaptMobileKeyBoard.Create(new RectTransform[] { move_rect }, this, _up_offset);
        }

        /// <summary>
        /// 是否开启虚拟键盘弹出时上移界面
        /// </summary>
        public void CreateViewAdaptKeyBoard(RectTransform[] move_rects, float _up_offset = 0)
        {
            ChatViewAdaptMobileKeyBoard.Create(move_rects, this, _up_offset);
        }
    }
}
