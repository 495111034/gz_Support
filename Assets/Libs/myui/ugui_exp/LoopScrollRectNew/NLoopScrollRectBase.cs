...
namespace UnityEngine.UI
{
    ...
    /// </remarks>
    public abstract class NLoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        public Action<Transform, int> ProvideDataEvent = null;

        public GameObject prefab;

        [Header("节点动画间隔")]
        public float cellAnimaInterval = 0;

        public bool OnValueChangedMessage { set; get; }
        /// <summary>
        ...
        [HideInInspector]
        [NonSerialized]
        public Func<int, Vector2> CustomGetItemsSize = null;
        ...
        private bool is_layout_center = false;
        private HorizontalOrVerticalLayoutGroup layout1;
        [NonSerialized] public bool AutoSetCenter = true;
        ...
        protected float contentSpacing
        {
            get
            {
                ...
                if (m_Content != null)
                {
                    layout1 = m_Content.GetComponent<HorizontalOrVerticalLayoutGroup>();
                    if (layout1 != null)
                    {
...
#if UNITY_EDITOR
                    if (layout1 == null && m_GridLayout == null)
                    {
                        Debug.LogError("未找到布局组件，如果使用MyGridLayoutGroup请换成Unity原来的");
                    }
#endif
                }
                return m_ContentSpacing;
        ...
        protected override void Awake()
        {
            ...
            if (Application.isPlaying)
            {
                if (prefab != null)
                {
                    prefab.transform.SetParent(content.parent);
                    prefab.SetActive(false);
                }
                layout1 = m_Content.GetComponent<HorizontalOrVerticalLayoutGroup>();
                if (layout1 != null)
                {
                    is_layout_center = layout1.childAlignment == TextAnchor.MiddleCenter;
                }
            }
            ...
            if (Application.isPlaying)
            {
                ...
                Debug.Assert(Mathf.Approximately(GetAbsDimension(m_Content.pivot), value), $"m_Content.pivot set error{m_Content.pivot}, right is {value}");
                Debug.Assert(Mathf.Approximately(GetAbsDimension(m_Content.anchorMin), value), $"m_Content.anchorMin set error{m_Content.anchorMin}, right is {value}");
                Debug.Assert(Mathf.Approximately(GetAbsDimension(m_Content.anchorMax), value), $"m_Content.anchorMax set error{m_Content.anchorMax}, right is {value}");
        ...
        public int GetFirstItem(out float offset)
        {
            ...
            if (itemTypeEnd > itemTypeStart)
            {
                ...
                {
                    offset += size;
                    offset += contentSpacing; //Add ContentSpacing
        ...
        public void SrollToCellWithinTime(int index, float time)
        {
            ...
            {
                if (CustomGetItemsSize != null)
                {
                    dist = GetDimension(CustomGetItemsSize(currentFirst) - CustomGetItemsSize(index));
                    dist += offset;
                }
                else
                {
        ...
        /// <summary>
        /// 是否是无限滚动模式，true则不判断index溢出totalCount，false则需判断index溢出totalCount时跳出循环，因为滚动时改变列表会导致无法跳出循环
        /// </summary>
        public bool is_infinite_scroll { set; get; } = false;
        ...
        public void RefreshCells(int count, float t)
        {
            _total_count = count;
            if (!IsInvoking("TickUpdateScrollView"))
            {
                this.totalCount = 0;
                this.RefillCells();
                InvokeRepeating("TickUpdateScrollView", t, t);
            }
        }

        private void TickUpdateScrollView()
        {
            if (this.totalCount != _total_count)
            {
                if (this.totalCount < _total_count) this.totalCount++;
                else this.totalCount--;
                this.RefreshCells();
            }
            else
            {
                CancelInvoke("TickUpdateScrollView");
                this.RefillCells();
            }
        }
        ...
        public void RefillCellsFromEnd(int endItem = 0, bool alignStart = false)
        {
            ...
            int dead_loop_check = 0;

            bool first = true;
            while (sizeToFill > sizeFilled && dead_loop_check < sizeToFill)
            {
            ...
            LayoutRebuilderEx.ForceRebuildLayoutImmediate(m_Content);
            ...
            if (is_layout_center)
            {
                if (!IsInvoking("CheckCenterLayout"))
                {
                    Invoke("CheckCenterLayout", Time.deltaTime);
                }
            }
        }
        ...
        private void CheckCenterLayout()
        {
            if (!AutoSetCenter) return;
            if (is_layout_center)
            {
                if (direction == LoopScrollRectDirection.Horizontal && m_Content.sizeDelta.x < m_ViewBounds.size.x)
                {
                    if (layout1 != null)
                    {
                    ...
                    this.enabled = false;
                    m_Content.anchoredPosition += Vector2.right * (m_ViewBounds.size.x - m_Content.sizeDelta.x) * 0.5f;
                }
                else if (direction == LoopScrollRectDirection.Vertical && m_Content.sizeDelta.y < m_ViewBounds.size.y)
                {
                    if (layout1 != null)
                    {
                        layout1.childAlignment = TextAnchor.MiddleCenter;
                    ...
                    this.enabled = false;
                    m_Content.anchoredPosition -= Vector2.up * (m_ViewBounds.size.y - m_Content.sizeDelta.y) * 0.5f;
                }
                else
                {
                    if (layout1 != null)
                    {
                        layout1.childAlignment = TextAnchor.MiddleLeft;
                    }
                    if (!this.enabled)
                    {
                        this.enabled = true;
        ...
        protected void ReturnObject(Transform trans)
        {
        ...
        public virtual void Rebuild(CanvasUpdate executing)
        {
            ...
            //{
            //    UnityEngine.Profiling.Profiler.BeginSample(ProfileName);
            //}
            if (executing == CanvasUpdate.Prelayout)
            {
                UnityEngine.Profiling.Profiler.BeginSample("UpdateCachedData");
                UpdateCachedData();
            ...
            if (executing == CanvasUpdate.PostLayout)
            {
                ...
                UnityEngine.Profiling.Profiler.BeginSample("UpdateScrollbars");
                UpdateScrollbars(Vector2.zero);
                UnityEngine.Profiling.Profiler.EndSample();

                UnityEngine.Profiling.Profiler.BeginSample("UpdatePrevData");
                UpdatePrevData();
                UnityEngine.Profiling.Profiler.EndSample();
        ...
        private void EnsureLayoutHasRebuilt()
        {
            ...
            {
                //Log.LogInfo($"{ProfileName} m_HasRebuiltLayout={m_HasRebuiltLayout}, IsRebuildingLayout={CanvasUpdateRegistry.IsRebuildingLayout()}");
                m_HasRebuiltLayout = true;//避免重复调用 ForceUpdateCanvases
                Canvas.ForceUpdateCanvases();
            }
        }
        ...
        protected virtual void LateUpdate()
        {
            ...
            UnityEngine.Profiling.Profiler.BeginSample(ProfileName);
            {
                ...
                UnityEngine.Profiling.Profiler.BeginSample("CalculateOffset");
                ...
                if (!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
                {
                    ...
                    if (m_MovementType == MovementType.Clamped)
                    {
                        UnityEngine.Profiling.Profiler.BeginSample("CalculateOffset2");
                        offset = CalculateOffset(position - m_Content.anchoredPosition);
                        UnityEngine.Profiling.Profiler.EndSample();
                        position += offset;
                    }

                    UnityEngine.Profiling.Profiler.BeginSample("SetContentAnchoredPosition");
                    SetContentAnchoredPosition(position);
                    UnityEngine.Profiling.Profiler.EndSample();
                }
                ...
                if (m_ViewBounds != m_PrevViewBounds || m_ContentBounds != m_PrevContentBounds || m_Content.anchoredPosition != m_PrevPosition)
                {
                    ...
                    UnityEngine.Profiling.Profiler.BeginSample("ScrollRect.value");
                    UISystemProfilerApi.AddMarker("ScrollRect.value", this);
                    UnityEngine.Profiling.Profiler.EndSample();
#endif
                    UnityEngine.Profiling.Profiler.BeginSample("m_OnValueChanged");
                    m_OnValueChanged.Invoke(normalizedPosition);
                    UnityEngine.Profiling.Profiler.EndSample();

                    if (OnValueChangedMessage)
                    {
                        UnityEngine.Profiling.Profiler.BeginSample("_onScrollRectValueChanged");
        ...
        private void GetHorizonalOffsetAndSize(out float totalSize, out float offset)
        {
            if (CustomGetItemsSize != null)
            {
                totalSize = CustomGetItemsSize(TotalLines).x;
                offset = m_ContentBounds.min.x - CustomGetItemsSize(StartLine).x - contentSpacing * StartLine;
            }
            else
            {
        ...
        private void GetVerticalOffsetAndSize(out float totalSize, out float offset)
        {
            if (CustomGetItemsSize != null)
            {
                totalSize = CustomGetItemsSize(TotalLines).y;
                offset = m_ContentBounds.max.y + CustomGetItemsSize(StartLine).y + contentSpacing * StartLine;
            }
            else
            {
