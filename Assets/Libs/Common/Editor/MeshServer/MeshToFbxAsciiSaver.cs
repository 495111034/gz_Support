...
namespace GameSupportEditor
{
    public partial class MeshToFbxAsciiSaver : IMeshSaver
    {
        ...
        public void Save(Mesh m, Material mat, string path , string fileName)
        {
            fbxFileName = fileName;
            mesh = m;
            material = mat;
            StringBuilder sb = new StringBuilder();
            sb.Append(CreateHeader()).Append(Token.EOL);
            sb.Append(CreateObjectsDefinition()).Append(Token.EOL);
            sb.Append(CreateObjects()).Append(Token.EOL);
            sb.Append(CreateObjectConnections()).Append(Token.EOL);
        ...
        private FbxNode CreateModelNode()
        {
            ...
            modelNode.AddProperty("Version", 232);
            modelNode.AddProperty("MultiLayer", 0);
            modelNode.AddProperty("Shading", "Y");
            modelNode.AddProperty("Culling", "\"Culling Off\"");
            modelNode.AddProperty("Vertices", ListVertices());
            modelNode.AddProperty("PolygonVertexIndex", ListTriangles());
            modelNode.AddProperty("GeometryVersion", 124);
            modelNode.AddSubNode(CreateNormalsNode());
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                modelNode.AddSubNode(CreateUvNode(i , uv));
            }

            if (mesh.colors != null && mesh.colors.Length > 0)
            {
                modelNode.AddSubNode(CreateColorsNode());
            }
            modelNode.AddSubNode(CreateMaterialNode());
            ...
            for (int i = 1; i < 8; i++)
            {
                ...
                if (uvs.Count > 0)
                {
                    modelNode.AddSubNode(CreateUVLayerNode(i));
                }
            }
            return modelNode;
        }
        ...
        private FbxNode CreateUvNode(int i , List<Vector2> uv)
        {
            FbxNode normalsNode = new FbxNode("LayerElementUV", i)
          .AddProperty("Version", 101)
          .AddProperty("Name", "\"UVMap\"")
          .AddProperty("MappingInformationType", "\"ByVertice\"")
          .AddProperty("ReferenceInformationType", "\"Direct\"")
          .AddProperty("UV", ListUVs(uv));
            return normalsNode;
            ...
            //    .AddProperty("MappingInformationType", "\"ByVertice\"")
            //    .AddProperty("ReferenceInformationType", "\"Direct\"")
            //    .AddProperty("UV", ListUVs(mesh.uv));
            //return normalsNode;
        }
        ...
        private FbxNode CreateLayerNode()
        {
            ...
            layerNode.AddProperty("Version", 100);
            layerNode.AddSubNode(normalLayerElementNode);
            layerNode.AddSubNode(uvLayerElementNode);
            layerNode.AddSubNode(colorLayerElementNode);
            layerNode.AddSubNode(materialLayerElementNode);
            layerNode.AddSubNode(textureLayerElementNode);

            return layerNode;
        }

        private FbxNode CreateUVLayerNode(int i)
        {
            FbxNode uvLayerElementNode = new FbxNode("LayerElement", "")
                .AddProperty("Type", "\"LayerElementUV\"")
                .AddProperty("TypedIndex", i);

            FbxNode layerNode = new FbxNode("Layer", i);
            layerNode.AddProperty("Version", 100);
            layerNode.AddSubNode(uvLayerElementNode);
        ...
        private string ListUVs(List<Vector2> uvs)
        {
            StringBuilder sb = new StringBuilder();
