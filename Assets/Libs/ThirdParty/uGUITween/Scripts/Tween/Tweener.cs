...
namespace uTools
{
    public abstract class Tweener : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            UnityEngine.Profiling.Profiler.BeginSample(" Loop style");
            if (style == Style.Loop)
            {
                if (mFactor > 1f)
                {
            ...
            {
                UnityEngine.Profiling.Profiler.BeginSample("disable the script");
        ...
        {
            if (!isFinished && Time.unscaledTime - last_update_time < 0.03f)
            {
                //太频繁
                return;
            }            
            last_update_time = Time.unscaledTime; 
            ...
            {
                sample_name = $"{this.GetType()}_{gameObject.name}";
            }

            if(Application.isEditor) UnityEngine.Profiling.Profiler.BeginSample(sample_name);
            // Call the virtual update
            OnUpdate((method == EaseType.none) ? animationCurve.Evaluate(val) : val, isFinished);
            // onUpdate?.Invoke();          
            if(Application.isEditor) UnityEngine.Profiling.Profiler.EndSample();
        }
