...
namespace UnityEditor
{
    public class F4VectorDrawer : MaterialPropertyDrawer
    {
        ...
        string label3, float min3, float max3)
        {
            if (label0 != HIDE)
            {
                this.label0 = min0 < max0 ? string.Format("金属({0}) ({1}-{2})", label0, min0, max0) : label0;
                this.min0 = min0;
                this.max0 = max0;
            }
            else
            ...
            if (label1 != HIDE)
            {
                this.label1 = min1 < max1 ? string.Format("镜面({0}) ({1}-{2})", label1, min1, max1) : label1;
                this.min1 = min1;
                this.max1 = max1;
            }
            else
            ...
            if (label2 != HIDE)
            {
                this.label2 = min2 < max2 ? string.Format("菲涅尔强度({0}) ({1}-{2})", label2, min2, max2) : label2;
                this.min2 = min2;
                this.max2 = max2;
            }
            else
            ...
            if (label3 != HIDE)
            {
                this.label3 = min3 < max3 ? string.Format("菲涅尔范围({0}) ({1}-{2})", label3, min3, max3) : label3;
                this.min3 = min3;
                this.max3 = max3;
            }
            else
