...
namespace Coffee.UIExtensions
{
    ...
    public class UIParticle : MaskableGraphic, ISerializationCallbackReceiver
    {
        ...
        [SerializeField]
        private Vector3 m_Scale3D = Vector3.one;
        ...
        public void RefreshParticles(bool isHideParticleRender = true)
        {
            RefreshParticles(gameObject, isHideParticleRender);
        }

        private void RefreshParticles(GameObject root, bool isHideParticleRender = true)
        {
            if (!root) return;
            root.GetComponentsInChildren(true, particles);
            particles.RemoveAll(x => x.GetComponentInParent<UIParticle>(true) != this || x.subEmitters.enabled);
            ...
            RefreshParticles(particles, isHideParticleRender);
        }



        public void RefreshParticles(List<ParticleSystem> particles, bool isHideParticleRender = true)
        {
            // #246: Nullptr exceptions when using nested UIParticle components in hierarchy
            _renderers.Clear();
            foreach (ParticleSystem child in transform.GetComponentsInChildren<ParticleSystem>(true))
            {
                if (child.subEmitters.enabled)
                {
                    continue;
                }
                ...
                {
                    uiParticleRenderer = child.gameObject.AddComponent<UIParticleRenderer>();
                }
                if (!uiParticleRenderer.enabled)
                {
                    uiParticleRenderer.enabled = true;
                }
            ...
            for (var i = 0; i < _renderers.Count; i++)
            {
                _renderers[i].Reset(i, isHideParticleRender);
            }
            ...
            for (var i = 0; i < particles.Count; i++)
            {
                var ps = particles[i];
                if (!ps) continue;
                GetRenderer(j++).Set(this, ps, ps.trails.enabled, isHideParticleRender);
            }
        }
        ...
        internal void UpdateGroupId()
        {
            if(gameObject == null || transform.parent == null || canvas == null)
            {
                return ;
            }

            _groupId = gameObject.name.GetHashCode() + Mathf.CeilToInt((transform.parent.transform as RectTransform).rect.width) + canvas.sortingOrder; //同一界面，尺寸，名字才会共享网格
        }
