    ...
    internal class UIParticleEditor : GraphicEditor
    {
        ...
        public class HierarchyChangeListener
        {
            ...
            static void OnHierarchyWindowChanged()
            {
                ...
                // 当Hierarchy视图中的层次结构发生变化时，此方法将被调用
                //Debug.Log("Hierarchy changed in the Editor.");
                var particles = FindObjectsOfType<UIParticle>();
                for (int i = 0; i < particles.Length; i++)
                {
                    particles[i].RefreshParticles(false);
                }
            }
        }
        ...
        private static void Init()
        {
            ...
            for (int i = 0; i < particles.Length; i++)
            {
                TransfromExtensions.AddComponentFromPrefab<RectTransform>(particles[i].gameObject);
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(_meshSharing);
