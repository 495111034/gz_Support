Shader "hc/partical/AlphaAdd_4layers_mask_ui" {
Properties {	

	_FirstTex ("第一纹理", 2D) = "white" {}
    _FirstX ("第一纹理X速度", Range(-20,20)) = 0.0
    _FirstY ("第一纹理Y速度", Range(-20,20)) = 0.0
...
CGINCLUDE
	...
	v2f vert (appdata_t v)
	{
		...
		o.uv5 = v.texcoord.xy * _Mask_ST.xy + _Mask_ST.zw;			
	...
	fixed4 frag (v2f i) : SV_Target
	{
		...
		fixed4 _MaskTex =  tex2D(_Mask, i.uv5);
		...
		fixed4 firstLayer = fixed4(_FirstTexColor.rgb * _TintColor1.rgb * _FirstTexColor.a,0);				
		...
		fixed4 secondLayer = fixed4(_SecondTexColor.rgb * _TintColor2.rgb  * _SecondTexColor.a * clamp(1-_FirstTexColor.a,0,1),0);
		...
		fixed4 thirdLayer = fixed4(_ThirdTexColor.rgb * _TintColor3.rgb * _ThirdTexColor.a *  clamp(1-(_FirstTexColor.a + _SecondTexColor.a),0,1) ,0);
		...
		fixed4 fourthLayer =fixed4(_FourthTexColor.rgb * _TintColor4.rgb * _FourthTexColor.a *  clamp(1-(_FirstTexColor.a + _SecondTexColor.a + _ThirdTexColor.a),0,1) ,0);
		
		fixed4 col = 2.0 * i.color  * fixed4(fixed3(firstLayer.rgb + secondLayer.rgb + thirdLayer.rgb + fourthLayer.rgb),_MaskTex.a);
