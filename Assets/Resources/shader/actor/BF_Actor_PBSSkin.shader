...
* 文件简述：
*/
Shader "BF/Actor/PBSSkin"
{
    Properties{
		//Setting
		_Mode ("模式", Float) = 0
		[Enum(UnityEngine.Rendering.BlendMode)] _BlendSrcColor ("源颜色混合比例", Float) = 1.0
		[Enum(UnityEngine.Rendering.BlendMode)]_BlendDstColor ("混合颜色混合比例", Float) = 0.0
		...
		//Clip
		[Toggle(CLIP_ON)]_ClipOn("开启透明裁剪", float) = 0
		//_CullOffAlpha("裁剪透明度",Range(0.001,1)) = 0.1
        _ClipAlpha("裁剪透明度",Range(0.001,1)) = 0.1
		//Hollow
    ...
    SubShader{
        ...
        Pass{
            ...
            CGPROGRAM
				...
				#pragma multi_compile __ FRESNELRIM_ON //#pragma shader_feature_local _ FRESNELRIM_ON//优化  
				...
				float4 frag(v2f i) : SV_Target{
					...
					UnityIndirect gi;
					gi.diffuse = _GiDiffuse * ShadeSHPerPixel(worldNormal, i.vertexAmbient, worldPos) + (1 - _GiDiffuse);
					...
					//half4 color = BRDF2_BF_PBS(diffColor, specColor, oneMinusReflectivity, smoothness, worldNormal, i.worldViewDir, light, gi);    
					...
					if(_RimOn > 0.5)
					{
						half3 rimColor1 = pow(1 - surfaceData.nv, _RimPow) * _RimColor.rgb;
						color.rgb += rimColor1;
					}
				#endif
				#if _G_ACTOR_RIM2
					...
					if(_Rim2On > 0.5)
					{
						...
						half3 rimColor2 = _RimWidth2 * lerp(_RimSecondColor2.rgb, _RimColor2.rgb, nv1);
						color.rgb += rimColor2; 
					}
				#endif
					...
					float alpha = fade * (mainTex.a  -_FresnelScale2);
					return half4(color.rgb, alpha) + half4(_FresnelScale2, _FresnelScale2, _FresnelScale2, 0);
					//return half4(color.rgb, mainTex.a) +  half4(_FresnelScale2, _FresnelScale2, _FresnelScale2, -_FresnelScale2);
				}
            ENDCG
        }
