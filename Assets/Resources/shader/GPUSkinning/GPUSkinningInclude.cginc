...
inline float4 indexToUV(int pixelIndex)
{
	int columnCount = (int)_GPUSkinning_TextureSize_NumPixelsPerFrame.x;
	int rowIndex = pixelIndex / columnCount;
	int columnIndex = pixelIndex - rowIndex * columnCount;
	return float4(saturate(columnIndex / _GPUSkinning_TextureSize_NumPixelsPerFrame.x), saturate(rowIndex / _GPUSkinning_TextureSize_NumPixelsPerFrame.y), 0, 0);
}

inline float4x4 getMatrix(int frameStartIndex, int boneIndex)
{
	int matStartIndex = frameStartIndex + boneIndex * 3;
	half4 row0 = tex2Dlod(_GPUSkinning_TextureMatrix, indexToUV(matStartIndex));
	half4 row1 = tex2Dlod(_GPUSkinning_TextureMatrix, indexToUV(matStartIndex + 1));
	half4 row2 = tex2Dlod(_GPUSkinning_TextureMatrix, indexToUV(matStartIndex + 2));
...
#define textureMatrix_crossFade(uv2, uv3) \
	int frameStartIndex_crossFade = getFrameStartIndex_crossFade(); \
...
#define skin1_noroot(mat0, mat1, mat2, mat3) \
	mul(mat0, vertex) * uv2.y;

#define skin1_root(mat0, mat1, mat2, mat3, root) \
	mul(root, mul(mat0, vertex)) * uv2.y;

#define skin2_noroot(mat0, mat1, mat2, mat3) \
	mul(mat0, vertex) * uv2.y + \
	mul(mat1, vertex) * uv2.w;

#define skin2_root(mat0, mat1, mat2, mat3, root) \
	mul(root, mul(mat0, vertex)) * uv2.y + \
	mul(root, mul(mat1, vertex)) * uv2.w;

#define skin4_noroot(mat0, mat1, mat2, mat3) \
	mul(mat0, vertex) * uv2.y + \
	mul(mat1, vertex) * uv2.w + \
	mul(mat2, vertex) * uv3.y + \
	mul(mat3, vertex) * uv3.w;

#define skin4_root(mat0, mat1, mat2, mat3, root) \
	mul(root, mul(mat0, vertex)) * uv2.y + \
	mul(root, mul(mat1, vertex)) * uv2.w + \
	mul(root, mul(mat2, vertex)) * uv3.y + \
	mul(root, mul(mat3, vertex)) * uv3.w;

#define skin_blend(pos0, pos1) \
	pos1.xyz + (pos0.xyz - pos1.xyz) * crossFadeBlend

#define rootOff_BlendOff(quality) \
	textureMatrix(uv2, uv3); \
	return skin4_noroot(mat0, mat1, mat2, mat3);

#define rootOn_BlendOff(quality) \
	textureMatrix(uv2, uv3); \
	float4x4 root = rootMotion; \
	return skin4_root(mat0, mat1, mat2, mat3, root);

#define rootOn_BlendOn_CrossFadeRootOn(quality) \
	textureMatrix(uv2, uv3); \
...
#define rootOn_BlendOn_CrossFadeRootOff(quality) \
	textureMatrix(uv2, uv3); \
	textureMatrix_crossFade(uv2, uv3); \
	float4x4 root = rootMotion; \
	float4 pos0 = skin4_root(mat0, mat1, mat2, mat3, root); \
...
#define rootOff_BlendOn_CrossFadeRootOn(quality) \
	textureMatrix(uv2, uv3); \
	textureMatrix_crossFade(uv2, uv3); \
	float4x4 root_crossFade = rootMotion_crossFade; \
...
#define rootOff_BlendOn_CrossFadeRootOff(quality) \
	textureMatrix(uv2, uv3); \
	textureMatrix_crossFade(uv2, uv3); \
	float4 pos0 = skin4_noroot(mat0, mat1, mat2, mat3); \
	float4 pos1 = skin4_noroot(mat0_crossFade, mat1_crossFade, mat2_crossFade, mat3_crossFade); \
