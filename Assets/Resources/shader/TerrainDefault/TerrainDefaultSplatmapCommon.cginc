...
void SplatmapVert(inout appdata_full v, out Input data)
{
    ...
    UNITY_TRANSFER_FOG(data, worldPos);
#else
...
#endif
{
    ...
    float2 uvSplat3 = TRANSFORM_TEX(IN.tc.xy, _Splat3);
    ...
    #ifdef TERRAIN_STANDARD_SHADER
        fixed4 mainTex0 = tex2D(_Splat0, uvSplat0) * half4(1.0, 1.0, 1.0, defaultAlpha.r);
        fixed4 mainTex1 = tex2D(_Splat1, uvSplat1) * half4(1.0, 1.0, 1.0, defaultAlpha.g);
        fixed4 mainTex2 = tex2D(_Splat2, uvSplat2) * half4(1.0, 1.0, 1.0, defaultAlpha.b);
        fixed4 mainTex3 = tex2D(_Splat3, uvSplat3) * half4(1.0, 1.0, 1.0, defaultAlpha.a);
    #else
        fixed4 mainTex0 = tex2D(_Splat0, uvSplat0);
        fixed4 mainTex1 = tex2D(_Splat1, uvSplat1);
        fixed4 mainTex2 = tex2D(_Splat2, uvSplat2);
        fixed4 mainTex3 = tex2D(_Splat3, uvSplat3);
    #endif
        ...
        heightBlend = max(heightBlend - ma + _HeightBlendWeight, 0) * splat_control;

        splat_control = heightBlend / (heightBlend.r + heightBlend.g + heightBlend.b + heightBlend.a);
    #endif
    mixedDiffuse = splat_control.r * mainTex0 + splat_control.g * mainTex1
                    + splat_control.b * mainTex2 + splat_control.a * mainTex3;
