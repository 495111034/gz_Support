...
public class MeleeWeaponTrail : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        GameObject.DontDestroyOnLoad(_trailObject);

        _trailMesh = new Mesh();
        _trailMesh.name = "MeleeWeaponTrail";
    ...
    public void Init(Transform t_base, Transform t_tip, Material mat)
    {
        _base = t_base;
        _tip = t_tip;
        _trailObject.GetComponent<Renderer>().material = mat;
    }
    ...
    void Update()
    {
        ...
        if (pointsToUse.Count > 1)
        {
            ...
            for (int n = 0; n < pointsToUse.Count; ++n)
            {
                ...
                float time = (Time.time - p.timeCreated) / _lifeTime;   // 时间率 1->0

                Color color = Color.Lerp(Color.white, Color.clear, time);   // 过渡颜色
                if (_colors != null && _colors.Length > 0)      // 颜色动画
                {
                ...
                {
                    float sizeTime = time * (_sizes.Length - 1);    // 尺寸索引
                    ...
                    size = Mathf.Lerp(_sizes[(int)min], _sizes[(int)max], lerp);    // 获取尺寸插值
                }

                Vector3 lineDirection = p.tipPosition - p.basePosition; // 方向

                newVertices[n * 2] = p.basePosition - (lineDirection * (size * 0.5f));      // base 向后扩展
                newVertices[(n * 2) + 1] = p.tipPosition + (lineDirection * (size * 0.5f)); // tip 向前扩展

                newColors[n * 2] = newColors[(n * 2) + 1] = color;  // 颜色
